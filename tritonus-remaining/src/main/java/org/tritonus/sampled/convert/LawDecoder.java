...
import static org.tritonus.sampled.convert.LawEncoder.ALAW8;
import static org.tritonus.sampled.convert.LawEncoder.ALL;
import static org.tritonus.sampled.convert.LawEncoder.BIG_ENDIAN16;
import static org.tritonus.sampled.convert.LawEncoder.LAW_FORMATS;
import static org.tritonus.sampled.convert.LawEncoder.LITTLE_ENDIAN16;
import static org.tritonus.sampled.convert.LawEncoder.SIGNED8;
import static org.tritonus.sampled.convert.LawEncoder.ULAW8;
import static org.tritonus.sampled.convert.LawEncoder.UNSIGNED8;
import static org.tritonus.sampled.convert.LawEncoder.getConvertType;
import static org.tritonus.share.sampled.TConversionTool.alaw2pcm16;
import static org.tritonus.share.sampled.TConversionTool.alaw2pcm8;
...
import static org.tritonus.share.sampled.TConversionTool.ulaw2pcm16;
import static org.tritonus.share.sampled.TConversionTool.ulaw2pcm8;
...
//    }
    ...
    private static AudioFormat createTargetFormat(AudioFormat src, AudioFormat dst) {
    ...
    static class FromUlawStream extends TSynchronousFilteredAudioInputStream {
        ...
        protected int convert(byte[] inBuffer, byte[] outBuffer, int outByteOffset, int inFrameCount) {
            ...
            switch (convertType) {
            case UNSIGNED8:
                ulaw2pcm8(inBuffer, 0, outBuffer, outByteOffset, sampleCount, false);
                break;
            case SIGNED8:
                ulaw2pcm8(inBuffer, 0, outBuffer, outByteOffset, sampleCount, true);
                break;
            case BIG_ENDIAN16:
                ulaw2pcm16(inBuffer, 0, outBuffer, outByteOffset, sampleCount, true);
                break;
            case LITTLE_ENDIAN16:
                ulaw2pcm16(inBuffer, 0, outBuffer, outByteOffset, sampleCount, false);
                break;
            case ALAW8:
        ...
        protected void convertInPlace(byte[] buffer, int byteOffset, int frameCount) {
            ...
            switch (convertType) {
            case UNSIGNED8:
                ulaw2pcm8(buffer, byteOffset, sampleCount, false);
                break;
            case SIGNED8:
                ulaw2pcm8(buffer, byteOffset, sampleCount, true);
                break;
            case ALAW8:
            ...
            default:
                throw new RuntimeException(
                        "Call to convertInPlace, but it cannot convert in place. (convertType=" + convertType + ")");
            }
        }
    }
    ...
    static class FromAlawStream extends TSynchronousFilteredAudioInputStream {
        ...
        protected int convert(byte[] inBuffer, byte[] outBuffer, int outByteOffset, int inFrameCount) {
            ...
            switch (convertType) {
            case UNSIGNED8:
                alaw2pcm8(inBuffer, 0, outBuffer, outByteOffset, sampleCount, false);
                break;
            case SIGNED8:
                alaw2pcm8(inBuffer, 0, outBuffer, outByteOffset, sampleCount, true);
                break;
            case BIG_ENDIAN16:
                alaw2pcm16(inBuffer, 0, outBuffer, outByteOffset, sampleCount, true);
                break;
            case LITTLE_ENDIAN16:
                alaw2pcm16(inBuffer, 0, outBuffer, outByteOffset, sampleCount, false);
                break;
            case ULAW8:
        ...
        protected void convertInPlace(byte[] buffer, int byteOffset, int frameCount) {
            ...
            switch (convertType) {
            case UNSIGNED8:
                alaw2pcm8(buffer, byteOffset, sampleCount, false);
                break;
            case SIGNED8:
                alaw2pcm8(buffer, byteOffset, sampleCount, true);
                break;
            case ULAW8:
