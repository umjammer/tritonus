...
public class DspState {
    ...
    public vorbis_dsp_state getHandle() { return handle; }
    ...
    public int initAnalysis(Info info) {
        if (TDebug.TraceVorbisNative) { TDebug.out("initAnalysis(): begin"); }
        vorbis_info infoHandle = info.getHandle();
        int nReturn = CodecLibrary.INSTANCE.vorbis_analysis_init(handle, infoHandle);
        if (TDebug.TraceVorbisNative) { TDebug.out("initAnalysis(): end"); }
        return nReturn;
    }
    ...
    public int headerOut(
            ...
            Packet codePacket) {
        if (TDebug.TraceVorbisNative) { TDebug.out("headerOut(): begin"); }
        ...
        int nReturn = CodecLibrary.INSTANCE.vorbis_analysis_headerout(handle,
                ...
                commentPacketHandle,
                codePacketHandle);
        if (TDebug.TraceVorbisNative) { TDebug.out("headerOut(): end"); }
        return nReturn;
    }
    ...
    public int write(float[][] afValues, int nValues) {
        if (TDebug.TraceVorbisNative) { TDebug.out("write(): begin"); }
        Pointer bufferPointer = CodecLibrary.INSTANCE.vorbis_analysis_buffer(handle, nValues).getValue();
        if (TDebug.TraceVorbisNative) { TDebug.out(String.format("write(): bufferPointer: %s, %d", bufferPointer, nValues)); }
        if (afValues != null) {
            int nObjectArrayLength = afValues.length;
            if (TDebug.TraceVorbisNative) { TDebug.out(String.format("write(): objectArray length: %d", nObjectArrayLength)); }
            long bufferPointerP = 0;
            for (float[] floatArray : afValues) {
                if (TDebug.TraceVorbisNative) { TDebug.out(String.format("write(): floatArray: %d", floatArray.length)); }
        ...
        if (TDebug.TraceVorbisNative) { TDebug.out("write(): end"); }
        return nReturn;
    }
    ...
    public int flushPacket(Packet packet) {
        if (TDebug.TraceVorbisNative) { TDebug.out("flushPacket(): begin"); }
        ogg_packet packetHandle = packet.getHandle();
        int nReturn = CodecLibrary.INSTANCE.vorbis_bitrate_flushpacket(handle, packetHandle);
        if (TDebug.TraceVorbisNative) { TDebug.out("flushPacket(): end"); }
        return nReturn;
    }
    ...
    public int initSynthesis(Info info) {
        if (TDebug.TraceVorbisNative) { TDebug.out("initSynthesis(): begin"); }
        vorbis_info infoHandle = info.getHandle();
        int nReturn = CodecLibrary.INSTANCE.vorbis_synthesis_init(handle, infoHandle);
        if (TDebug.TraceVorbisNative) { TDebug.out("initSynthesis(): end"); }
        return nReturn;
    }
    ...
    public int blockIn(Block block) {
        if (TDebug.TraceVorbisNative) { TDebug.out("blockIn(): begin"); }
        vorbis_block blockHandle = block.getHandle();
        int nReturn = CodecLibrary.INSTANCE.vorbis_synthesis_blockin(handle, blockHandle);
        if (TDebug.TraceVorbisNative) { TDebug.out("blockIn(): end"); }
        return nReturn;
    }
    ...
    public int pcmOut(float[][] afPcm) {
        ...
        if (TDebug.TraceVorbisNative) { TDebug.out("pcmOut(): begin"); }
        int nSamples = CodecLibrary.INSTANCE.vorbis_synthesis_pcmout(handle, pcm);
        if (TDebug.TraceVorbisNative) { TDebug.out(String.format("pcmOut(): samples: %d", nSamples)); }
        if (nSamples > 0) {
            int nChannels = new vorbis_info(handle.vi).channels;
            if (TDebug.TraceVorbisNative) { TDebug.out(String.format("pcmOut(): channels: %d", nChannels)); }
            Pointer[] pp = pcm.getValue().getPointerArray(0, nChannels);
            for (int nChannel = 0; nChannel < nChannels; nChannel++) {
                afPcm[nChannel] = pp[nChannel].getFloatArray(0, nSamples);
                if (TDebug.TraceVorbisNative) { TDebug.out(String.format("pcmOut(): float array: %d", afPcm[nChannel].length)); }
            }
            if (TDebug.TraceVorbisNative) { TDebug.out("pcmOut(): end"); }
        }
        return nSamples;
    }
    ...
    public int read(int nSamples) {
        if (TDebug.TraceVorbisNative) { TDebug.out("read(): begin"); }
        int nReturn = CodecLibrary.INSTANCE.vorbis_synthesis_read(handle, nSamples);
        if (TDebug.TraceVorbisNative) { TDebug.out("read(): end"); }
        return nReturn;
    }
    ...
    public long getSequence() {
        if (TDebug.TraceVorbisNative) { TDebug.out("getSequence(): begin"); }
        long lReturn = handle.sequence;
        if (TDebug.TraceVorbisNative) { TDebug.out("getSequence(): end"); }
        return lReturn;
    }
