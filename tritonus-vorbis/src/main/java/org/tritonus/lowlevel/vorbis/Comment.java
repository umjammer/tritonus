...
public class Comment {
    ...
    public vorbis_comment getHandle() { return handle; }
    ...
    public void addComment(String strComment) {
        if (TDebug.TraceVorbisNative) { TDebug.out("addComment(): begin"); }
        CodecLibrary.INSTANCE.vorbis_comment_add(handle, strComment);
        if (TDebug.TraceVorbisNative) { TDebug.out("addComment(): end"); }
    }
    ...
    public void addTag(String strTag, String strComment) {
        if (TDebug.TraceVorbisNative) { TDebug.out("addTag(): begin"); }
        CodecLibrary.INSTANCE.vorbis_comment_add_tag(handle, strTag, strComment);
        if (TDebug.TraceVorbisNative) { TDebug.out("addTag(): end"); }
    }
    ...
    public int queryCount(String strTag) {
        if (TDebug.TraceVorbisNative) { TDebug.out("queryCount(): begin"); }
        int nReturn = CodecLibrary.INSTANCE.vorbis_comment_query_count(handle, strTag);
        if (TDebug.TraceVorbisNative) { TDebug.out("queryCount(): end"); }
        return nReturn;
    }
    ...
    public String[] getUserComments() {
        if (TDebug.TraceVorbisNative) { TDebug.out("getUserComments(): begin"); }
        ...
        if (TDebug.TraceVorbisNative) { TDebug.out("getUserComments(): end"); }
        return stringArray;
    }
    ...
    public String getVendor() {
        if (TDebug.TraceVorbisNative) { TDebug.out("getVendor(): begin"); }
        String strReturn = handle.vendor.getString(0);
        if (TDebug.TraceVorbisNative) { TDebug.out("getVendor(): end"); }
        return strReturn;
    }
