...
public abstract class AbstractPeriodicGenerator extends AbstractGenerator {
    ...
    protected FloatSampleBuffer generate() {
        ...
        int periodCount = Math.max(1, (int) (DELAY_IN_SECONDS / periodDurationInSeconds));

        int sampleCount = Math.round((getSampleRate() * periodCount) / getFrequency());
        FloatSampleBuffer floatSampleBuffer = new FloatSampleBuffer(1, sampleCount, getSampleRate());
        float amplitude = getAmplitude();
        float normalizedFrequency = getFrequency() / getSampleRate();
        float[] channelBuffer = floatSampleBuffer.getChannel(0);
        for (int i = 0; i < sampleCount; i++) {
            float sampleValue = amplitude * generateSample(normalizedFrequency * i);
     ...
     *
     * @param periodPosition the relative position inside a single period of the waveform.
     *                        The value passed is in the range [0.0, 1.0]
     * @return the sample value, scaled to [-1.0, +1.0]
     */
    protected abstract float generateSample(float periodPosition);
}
